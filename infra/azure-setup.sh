...
ssh -o StrictHostKeyChecking=no ubuntu@$LOADGEN_IP "cd /app && nohup /home/ubuntu/.local/bin/uv run locust -f locustfile.py --host http://$SERVER_PRIVATE_IP:8000 --web-host 0.0.0.0 --web-port 8089 > locust.log 2>&1 < /dev/null &"
